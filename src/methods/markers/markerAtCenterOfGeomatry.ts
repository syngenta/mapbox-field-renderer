...
export function addMarkerAtPoint(
  ...
  isDraggable: boolean = true,
     ...
     // Create a custom element for the marker using dragSvg or the provided custom element
     const dragElement = customElement || document.createElement("div");
     if (!customElement) {
       dragElement.innerHTML = dragSvg;
     }
 
     // Add a marker to the map with the custom icon
     const marker = new mapboxgl.Marker({ draggable: isDraggable, element: dragElement })
...
export function addRotationMarker(
  ...
  const initialLat = center[1] + maxDistance * Math.sin(Math.PI / 4);
   ...
   // Create a custom element for the marker using rotateSvg or the provided custom element
   const rotateElement = customElement || document.createElement("div");
   if (!customElement) {
